...
if __name__ == '__main__':
    ...
    if args.counterpartyd_rpc_port:
        ...
        if config.TESTNET:
            config.COUNTERPARTYD_RPC_PORT = 14411
        else:
            config.COUNTERPARTYD_RPC_PORT = 4411
    try:
    ...
    if args.rpc_port:
        ...
        if config.TESTNET:
            config.RPC_PORT = 14421
        else:
            config.RPC_PORT = 4421        
    try:
    ...
    if args.socketio_port:
        ...
        if config.TESTNET:
            config.SOCKETIO_PORT = 14121
        else:
            config.SOCKETIO_PORT = 4121        
    try:
    ...
    if args.socketio_chat_port:
        ...
        if config.TESTNET:
            config.SOCKETIO_CHAT_PORT = 14122
        else:
            config.SOCKETIO_CHAT_PORT = 4122       
    try:
    ...
    #More testnet
    if config.TESTNET:
        config.BLOCK_FIRST = 165000
    else:
        config.BLOCK_FIRST = 166000
