...
def get_price_primatives(start_dt=None, end_dt=None):
    ...
    xcp_btc_price = mps_xcp_btc['market_price'] if mps_xcp_btc else None # == SHP/SCH
    btc_xcp_price = calc_inverse(mps_xcp_btc['market_price']) if mps_xcp_btc else None #SCH/SHP
...
def get_asset_info(asset, at_dt=None):
    ...
    #modify some of the properties of the returned asset_info for SCH and SHP
    if asset == config.BTC:
        if at_dt:
        ...
        # as SHP doesn't deflate quickly at all, and shouldn't matter that much since there weren't any/much trades
...
def get_xcp_btc_price_info(asset, mps_xcp_btc, xcp_btc_price, btc_xcp_price, with_last_trades=0, start_dt=None, end_dt=None):
    if asset not in [config.BTC, config.XCP]:
        #get price data for both the asset with SHP, as well as SCH
        ...
        #aggregated (averaged) price (expressed as SHP) for the asset on both the SHP and SCH markets
        ...
        #here we take the normal SHP/SCH pair, and invert it to SCH/SHP, to get SHP's data in terms of a SCH base
        # (this is the only area we do this, as SCH/SHP is NOT standard pair ordering)
...
def compile_summary_market_info(asset, mps_xcp_btc, xcp_btc_price, btc_xcp_price):
    ...
    NOTE: in_btc == base asset is SCH, in_xcp == base asset is SHP
    ...
    return {
        'price_in_{}'.format(config.XCP.lower()): price_in_xcp, #current price of asset vs SHP (e.g. how many units of asset for 1 unit SHP)
        'price_in_{}'.format(config.BTC.lower()): price_in_btc, #current price of asset vs SCH (e.g. how many units of asset for 1 unit SCH)
        'price_as_{}'.format(config.XCP.lower()): calc_inverse(price_in_xcp) if price_in_xcp else None, #current price of asset AS SHP
        'price_as_{}'.format(config.BTC.lower()): calc_inverse(price_in_btc) if price_in_btc else None, #current price of asset AS SCH
...
def compile_24h_market_info(asset):
    ...
    #TOTAL volume and count across all trades for the asset (on ALL markets, not just SHP and SCH pairings)
    ...
    return {
        ...
        #^ quantity of asset traded with SCH in last 24h
        '24h_ohlc_in_{}'.format(config.BTC.lower()): _24h_ohlc_in_btc,
        #^ quantity of asset traded with SHP in last 24h
...
def compile_7d_market_info(asset):
    ...
    #get SHP and SCH market summarized trades over a 7d period (quantize to hour long slots)
    _7d_history_in_xcp = None # SHP/asset market (or SHP/SCH for SHP or SCH)
    _7d_history_in_btc = None # SCH/asset market (or SCH/SHP for SHP or SCH)
    if asset not in [config.BTC, config.XCP]:
    ...
    else: #get the SHP/SCH market and invert for SCH/SHP (_7d_history_in_btc)
        _7d_history = mongo_db.trades.aggregate([
            {"$match": {
                "base_asset": config.XCP,
...
def compile_asset_pair_market_info():
    ...
    #COMPOSE volume data (in SHP and SCH), and % change data
    ...
    #compose price data, relative to SCH and SHP
    ...
    for pair, e in pair_data.iteritems():
        ...
        #derive asset price data, expressed in SCH and SHP, for the given volumes
        if base_asset == config.XCP:
            ...
            if _24h_vol_in_xcp is None or _24h_vol_in_btc is None:
                #the base asset didn't have price data against SCH or SHP, or both...try against the quote asset instead
                price_summary_in_xcp, price_summary_in_btc, price_in_xcp, price_in_btc, aggregated_price_in_xcp, aggregated_price_in_btc = \
...
def compile_asset_market_info():
    ...
    #######################
    #get a list of all assets with a trade within the last 24h (not necessarily just against SHP and SCH)
    ...
    #######################
    #get a list of all assets with a trade within the last 7d up against SHP and SCH
    start_dt_7d = datetime.datetime.utcnow() - datetime.timedelta(days=7)
    assets = list(set(
    ...
    #get assets that were traded since the last check with either SCH or SHP, and update their market summary data
    assets = list(set(
