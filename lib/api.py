...
def serve_api(mongo_db, redis_client):
    ...
    def get_normalized_balances(addresses):
        ...
        NOTE: Does not retrieve SCH balance. Use get_address_info for that.
        """
        if not isinstance(addresses, list):
            raise Exception("addresses must be a list of addresses, even if it just contains one address")
    ...
    def get_market_info_leaderboard(limit=100):
        """returns market leaderboard data for both the SHP and SCH markets"""
        #do two queries because we limit by our sorted results, and we might miss an asset with a high SCH trading value
        # but with little or no SHP trading activity, for instance if we just did one query
    ...
    ask_book_min_pct_fee_provided=None, ask_book_min_pct_fee_required=None, ask_book_max_pct_fee_required=None):
        ...
        @param: normalized_fee_required: Only specify if buying SCH. If specified, the order book will be pruned down to only
         show orders at and above this fee_required
        @param: normalized_fee_provided: Only specify if selling SCH. If specified, the order book will be pruned down to only
        ...
        if base_asset == config.BTC or quote_asset == config.BTC:
            extra_filters = [
                {'field': 'give_remaining', 'op': '>', 'value': 0}, #don't show empty SCH orders
                {'field': 'get_remaining', 'op': '>', 'value': 0}, #don't show empty SCH orders
        ...
        #filter results by pct_fee_provided and pct_fee_required for SCH pairs as appropriate
        ...
        #for orders where SCH is the give asset, also return online status of the user
        for o in orders:
    ...
    def get_order_book_buysell(buy_asset, sell_asset, pct_fee_provided=None, pct_fee_required=None):
        ...
        if base_asset == config.BTC:
            if buy_asset == config.BTC:
                #if SCH is base asset and we're buying it, we're buying the BASE. we require a SCH fee (we're on the bid (bottom) book and we want a lower price)
                # - show BASE buyers (bid book) that require a SCH fee >= what we require (our side of the book)
                # - show BASE sellers (ask book) that provide a SCH fee >= what we require
                ...
                #if SCH is base asset and we're selling it, we're selling the BASE. we provide a SCH fee (we're on the ask (top) book and we want a higher price)
                # - show BASE buyers (bid book) that provide a SCH fee >= what we provide
                # - show BASE sellers (ask book) that require a SCH fee <= what we provide (our side of the book)
            ...
            if buy_asset == config.BTC:
                #if SCH is quote asset and we're buying it, we're selling the BASE. we require a SCH fee (we're on the ask (top) book and we want a higher price)
                # - show BASE buyers (bid book) that provide a SCH fee >= what we require
                # - show BASE sellers (ask book) that require a SCH fee >= what we require (our side of the book)
                ...
                #if SCH is quote asset and we're selling it, we're buying the BASE. we provide a SCH fee (we're on the bid (bottom) book and we want a lower price)
                # - show BASE buyers (bid book) that provide a SCH fee >= what we provide (our side of the book)
                # - show BASE sellers (ask book) that require a SCH fee <= what we provide
    ...
    def record_btc_open_order(wallet_id, order_tx_hash):
        """Records an association between a wallet ID and order TX ID for a trade where SCH is being SOLD, to allow
        buyers to see which sellers of the SCH are "online" (which can lead to a better result as a BTCpay will be required
        to complete any trades where SCH is involved, and the seller (or at least their wallet) must be online for this to happen"""
